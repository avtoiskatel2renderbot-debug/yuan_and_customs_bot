...
(
    ...
    ADD_CAR_COLOR, ADD_CAR_CLIENT, ADD_CAR_CLIENT_TYPE,
    DELETE_CAR_CONFIRM,
...
def get_spreadsheet():
    """
    Подключается к Google Sheets один раз.
    При повторных вызовах возвращает уже открытое соединение.
    Это главное ускорение — не тратим 2-3 сек на каждый запрос.
    """
...
def get_worksheet(name):
    ...
    try:
        ...
        # Сбрасываем соединение чтобы переподключиться
        global _spreadsheet
        _spreadsheet = None
    return None

def get_next_id(sheet_name, prefix):
    """Генерирует следующий ID типа AUTO-001, PAY-002 и т.д."""
    try:
        ws = get_worksheet(sheet_name)
        if not ws:
            return f"{prefix}-001"
        values = ws.col_values(1)[1:]  # Первый столбец без заголовка
...
def get_all_cars():
    """Получить все машины из таблицы"""
    try:
...
def build_duty_table():
    ...
    for v in volumes:
        ...
        text += f"{v:<5}  {rate}€/см³  {format_money(duty_rub):>10} ₽\n"
    text += "```\n\n"
...
2️⃣ Введи марку и модель одним сообщением:
   Пример: *Zeekr 001* или *Haval H6*
3️⃣ Введи год: *2024*
4️⃣ Введи цвет: *Белый*
5️⃣ Введи имя клиента:
   • Физлицо → *Иванов Иван Иванович*
   • Юрлицо → *ООО Автомир*
6️⃣ Выбери тип кнопкой: 👤 Физлицо или 🏢 Юрлицо

*🗑 Удалить машину:*
• Удаляй только после закрытия всех долгов
• При удалении стираются ВСЕ данные по машине:
  платежи, долги — всё
• Бот спросит подтверждение перед удалением
"""
...
• 💵 Допы от клиента — клиент заплатил (рубли ₽)

📤 *РАСХОДЫ:*
• 🚛 Автовоз — доставка по России (рубли ₽)
• 🏛 Таможенный брокер (рубли ₽)
• 🔧 Допы в Китае (юани ¥)
• ⛽ Бензин (рубли ₽)
• 💸 Кэшбэк юрику (юани ¥)
• 👤 % Менеджеру — 20 000₽ фикс (рубли ₽)

3️⃣ Введи сумму цифрами: *2800*
4️⃣ Комментарий или напиши *-* чтобы пропустить
...
• Клиент заказал допы но не заплатил
• Мы заказали допы в Китае но не оплатили
• Дилер заработал кэшбэк — нужно выплатить
• Менеджер привёл клиента — должны 20 000₽
• Ещё не оплатили брокера или автовоз

*➕ Добавить долг:*
1️⃣ Выбери машину
2️⃣ Выбери тип долга
3️⃣ Введи сумму

*✅ Закрыть долг (когда оплатили):*
1️⃣ Нажми ✅ Закрыть долг
2️⃣ Выбери нужный долг
3️⃣ Долг отмечается как оплачен

❗ После закрытия долга не забудь
записать платёж в разделе ➕ Платёж
"""

INSTRUCTION_SAL = """
📖 *ИНСТРУКЦИЯ — ЗАРПЛАТЫ*

Учёт зарплат и бонусов сотрудников.

*➕ Добавить зарплату:*
1️⃣ Имя сотрудника: *Иванов Кирилл*
2️⃣ Оклад: *30000*
3️⃣ Бонус: *5000* (если нет — напиши *0*)
4️⃣ Месяц: *07.2026*
Итого считается автоматически.

*✅ Отметить выплату:*
1️⃣ Нажми ✅ Отметить выплату
2️⃣ Выбери сотрудника
3️⃣ Бот ставит дату выплаты
...
Используй: в конце дня для проверки

*📅 За неделю* — платежи за 7 дней
Используй: еженедельная сверка

*📅 За месяц* — платежи текущего месяца
Используй: в конце месяца

*🚗 По машине* — все платежи и долги
по одному конкретному автомобилю
Используй: чтобы видеть полную картину сделки

*📈 P&L* — прибыль и убытки за месяц
Показывает доходы, расходы, зарплаты, итог
Используй: раз в месяц для общей картины

*⚖️ Все долги* — список открытых долгов
Используй: перед выплатами
...
async def show_instruction(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    back_to_inst = [InlineKeyboardButton("◀️ К инструкции", callback_data="inst_main")]
    ...
    instructions = {
        ...
        "inst_cars": (INSTRUCTION_CARS, [
            [back_to_inst[0]],
            [InlineKeyboardButton("🚗 Перейти в Машины", callback_data="fin_cars")],
        ]),
        ...
        "inst_debts": (INSTRUCTION_DEBTS, [
            [back_to_inst[0]],
            [InlineKeyboardButton("⚖️ Перейти в Долги", callback_data="fin_debts")],
        ]),
        "inst_sal": (INSTRUCTION_SAL, [
            [back_to_inst[0]],
            [InlineKeyboardButton("👥 Перейти в Зарплаты", callback_data="fin_sal")],
        ]),
        "inst_reports": (INSTRUCTION_REPORTS, [
            [back_to_inst[0]],
            [InlineKeyboardButton("📊 Перейти в Отчёты", callback_data="fin_reports")],
        ]),
    }
...
async def fin_cars(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [
        ...
        [InlineKeyboardButton("🗑 Удалить машину", callback_data="delete_car")],
        [InlineKeyboardButton("❓ Инструкция", callback_data="inst_cars")],
    ...
    if not cars:
        text = (
            ...
            "💡 После добавления машины ты сможешь\n"
            "записывать платежи и долги по ней."
        )
    else:
        text = f"🚗 *Машины* — всего в работе: {len(cars)}\n\n"
        for car in cars[-10:]:
            text += (
                f"*{car.get('ID', '—')}* — "
                f"{car.get('Марка', '—')} "
...
async def add_car_mark_model(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def add_car_client_type(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ws = get_worksheet("МАШИНЫ")
        if not ws:
            raise Exception("Не удалось подключиться к таблице")
        ...
        kb = [
            [InlineKeyboardButton("➕ Добавить платёж", callback_data="fin_pay")],
            [InlineKeyboardButton("◀️ К машинам", callback_data="fin_cars")],
        ]
        await query.edit_message_text(
            f"✅ *Машина добавлена!*\n\n"
            f"🆔 Номер: *{car_id}*\n"
            ...
            f"Теперь можно добавить платежи по этой машине.",
            reply_markup=InlineKeyboardMarkup(kb),
            parse_mode="Markdown"
        )
    except Exception as e:
        logger.error(f"add_car_client_type error: {e}")
        global _spreadsheet
        _spreadsheet = None  # Сбрасываем соединение
        kb = [[InlineKeyboardButton("◀️ Назад", callback_data="fin_cars")]]
        await query.edit_message_text(
            "❌ *Ошибка при сохранении.*\n\n"
            "Попробуйте ещё раз. Если ошибка повторяется — "
    ...
    kb = [
        [
            InlineKeyboardButton(
                "🗑 Да, удалить всё", callback_data=f"delconfirm_{car_id}"
            )
        ],
        [InlineKeyboardButton("❌ Отмена", callback_data="fin_cars")],
    ]
...
async def delete_car_execute(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    ...
    try:
        ...
        if car_ws:
            car_records = car_ws.get_all_records()
            for i, r in enumerate(car_records):
                if r.get("ID") == car_id:
        ...
        if pay_ws:
            pay_records = pay_ws.get_all_records()
            # Удаляем с конца чтобы не сбивать индексы
            rows_to_delete = [
                i + 2 for i, r in enumerate(pay_records)
                if r.get("ID машины") == car_id
            ]
            for row_num in sorted(rows_to_delete, reverse=True):
                pay_ws.delete_rows(row_num)

        # Удаляем долги
        debt_ws = get_worksheet("ДОЛГИ")
        if debt_ws:
            debt_records = debt_ws.get_all_records()
            rows_to_delete = [
                i + 2 for i, r in enumerate(debt_records)
                if r.get("ID машины") == car_id
            ]
            for row_num in sorted(rows_to_delete, reverse=True):
                debt_ws.delete_rows(row_num)

        kb = [[InlineKeyboardButton("◀️ К машинам", callback_data="fin_cars")]]
        await query.edit_message_text(
            f"✅ *Машина {car_id} удалена.*\n\n"
            f"Все платежи и долги по ней тоже удалены.",
            reply_markup=InlineKeyboardMarkup(kb),
            parse_mode="Markdown"
        )
    except Exception as e:
...
async def fin_pay_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not cars:
        ...
        await query.edit_message_text(
            ...
            "Платёж привязывается к конкретной машине.",
            reply_markup=InlineKeyboardMarkup(kb),
            parse_mode="Markdown"
        )
        return ConversationHandler.END
...
async def pay_car_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        f"Машина: *{car_id}*\n\n"
        "Шаг 2 из 3 — Выберите категорию платежа:",
...
async def pay_category_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cat_name == "% Менеджеру":
        ...
        await query.edit_message_text(
            ...
            f"_Напишите текст или «-» чтобы пропустить_",
            reply_markup=InlineKeyboardMarkup(kb),
            parse_mode="Markdown"
        )
    else:
...
async def pay_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "_Напишите текст или «-» чтобы пропустить_",
        reply_markup=InlineKeyboardMarkup(kb),
        parse_mode="Markdown"
    )
    return PAY_COMMENT
...
# ===== ТИПЫ ДОЛГОВ =====
DEBT_TYPES = [
    ("👤 Клиент должен нам за допы (рубли ₽)",
     "Клиент", "Нам", "RUB"),
    ("🏢 Мы должны клиенту — переплата (рубли ₽)",
     "Мы", "Клиенту", "RUB"),
    ("🇨🇳 Мы должны поставщику за допы (юани ¥)",
     "Мы", "Поставщику", "CNY"),
    ("💸 Мы должны юрику кэшбэк (юани ¥)",
     "Мы", "Юрику", "CNY"),
    ("👤 Мы должны менеджеру % (рубли ₽)",
     "Мы", "Менеджеру", "RUB"),
    ("🏛 Мы должны брокеру (рубли ₽)",
     "Мы", "Брокеру", "RUB"),
    ("🚛 Мы должны автовозу (рубли ₽)",
     "Мы", "Автовозу", "RUB"),
]
...
async def fin_debts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        kb = [
            ...
            [InlineKeyboardButton("❓ Инструкция", callback_data="inst_debts")],
            [InlineKeyboardButton("◀️ Назад", callback_data="finance_menu")],
        ]
        if not unpaid:
            text = "⚖️ *Долги*\n\n✅ Все долги погашены!"
...
async def debt_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            ...
            f"📌 Статус: ❌ Не оплачен\n\n"
            f"_Когда оплатишь — нажми ✅ Закрыть долг_",
            reply_markup=InlineKeyboardMarkup(kb),
            parse_mode="Markdown"
        )
    except Exception as e:
...
async def fin_sal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        kb = [
            ...
            [InlineKeyboardButton("❓ Инструкция", callback_data="inst_sal")],
            [InlineKeyboardButton("◀️ Назад", callback_data="finance_menu")],
        ]
        if not records:
            text = (
            ...
            for r in records[-8:]:
                ...
                text += (
                    f"{si} *{r.get('Сотрудник')}* — {r.get('Месяц')}\n"
                    f"Оклад: {r.get('Оклад')} ₽ | "
                    f"Бонус: {r.get('Бонус')} ₽ | "
                    f"Итого: *{r.get('Итого')} ₽*\n\n"
                )
...
async def report_month(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        kb = [[InlineKeyboardButton("◀️ К отчётам", callback_data="fin_reports")]]
        await query.edit_message_text(
            build_period_report(filtered, f"Отчёт за {now.strftime('%m.%Y')}"),
            reply_markup=InlineKeyboardMarkup(kb),
            parse_mode="Markdown"
        )
    except Exception as e:
...
async def report_pl(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        text += f"─────────────────\n"
        text += f"💵 *Итого рубли: {income_rub - expense_rub - sal_total:,.0f} ₽*\n"
        text += f"💴 *Итого юани: {income_cny - expense_cny:,.0f} ¥*"
...
def main():
    ...
    # Прогреваем соединение с Google Sheets при старте
    ...
    auth_conv = ConversationHandler(
        ...
        states={
            SET_PASSWORD: [MessageHandler(
                filters.TEXT & ~filters.COMMAND, handle_set_password
            )],
            ENTER_PASSWORD: [MessageHandler(
                filters.TEXT & ~filters.COMMAND, handle_enter_password
            )],
        },
    ...
    car_conv = ConversationHandler(
        ...
        states={
            ADD_CAR_MARK_MODEL: [MessageHandler(
                filters.TEXT & ~filters.COMMAND, add_car_mark_model
            )],
